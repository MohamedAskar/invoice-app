...
function updateInvoiceStatus(invoice: Invoice): Invoice {
  const persistedStatus = invoice.persistedStatus ?? invoice.status;
  if (invoice.status === 'paid' || invoice.status === 'draft') {
    return { ...invoice, persistedStatus };
  }

  if (isAfter(new Date(), parseISO(invoice.dueDate))) {
    return { ...invoice, status: 'overdue' as InvoiceStatus, persistedStatus };
  }
...
export const useInvoices = create<InvoicesStore>((set, get) => ({
  ...
  updateInvoice: async (invoice: Invoice) => {
    await saveInvoice(invoice);
    const updated = updateInvoiceStatus({ ...invoice, persistedStatus: invoice.status });
    set((state) => ({
