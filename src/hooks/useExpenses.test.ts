...
beforeEach(() => {
  vi.clearAllMocks();
  useExpenses.setState({ expenses: [expense()], loading: false, busy: false, error: undefined });
});
...
describe('useExpenses document lifecycle', () => {
  ...
  it('refreshes the visible detail after metadata removal leaves an orphan cleanup warning', async () => {
    storage.deleteExpenseDocument.mockRejectedValue(new Error('The document record was removed, but its private file needs cleanup.'));
